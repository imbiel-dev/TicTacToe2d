...
#exported variables for circle and cross scenes to be used as markers
...
func _ready():
	board_size = $Board.texture.get_width() #get the board size from the texture width
...
func new_game():
	...
	create_marker(player, player_panel_pos + Vector2i(cell_size / 2, cell_size / 2), true)
	#hide game over menu and unpause the game
	$GameOverMenu.hide()
	get_tree().paused = false

#function to create a marker for a player at a given position
...
#function to handle the game over menu restart signal
